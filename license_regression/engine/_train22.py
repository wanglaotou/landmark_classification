...
class TrainingEngine():
    ...
    def __init__(self, modelPath=None, imgListPath=None, classNum=8, batchSize=32, workers=4, imgChannel = 1, inputSize=[128, 128, 1]):
        ...
        self.loss = None
        # self.lossFn = ls.l2lossWithMulti4()
        self.lossFn = nn.CrossEntropyLoss().cuda()
    ...
    def train_batch_with_ang_multi3(self, data, idx):
        ...
        # print('out, label1:', out[0].size(), imgBatch.size(), label1.size(), label1)
        # out, label1: torch.Size([128, 8]) torch.Size([128, 3, 128, 128]) torch.Size([128])
   
        self.loss = self.lossFn(out[0], label1.long())
    ...
    def val_batch_with_ang_multi3(self):
        ...
        for idx_, data_ in enumerate(self.testdataLoader):
            ...
            with torch.no_grad():
                ...
                loss = self.lossFn(out[0], label1.long())
