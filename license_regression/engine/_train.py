...
class TrainingEngine():
    ...
    def __init__(self, modelPath=None, imgListPath=None, classNum=8, batchSize=32, workers=4, imgChannel = 3, inputSize=[128, 128, 1]):
        ...
        self.clsloss = nn.CrossEntropyLoss().cuda()  ## nn.CrossEntropyLoss().cuda()
    ...
    def train_batch_with_ang_multi3(self, data, idx):
        ...
        eyelabel1 = data[1].to(device)
        label1 = data[2].to(device)


        #print(imgBatch.size(), eyelabel1.size(), label2.size())  
        out = self.network(imgBatch)
        # print('out:', out[0].size(), out[1].size())
        # print(imgBatch.size(), eyelabel1.size(), label2.size(), out[0].size(), out[1].size())   
        self.loss1  = self.lossFn(out[0], eyelabel1)
        self.loss2 = self.clsloss(out[1], label1.long())
        self.loss = self.loss1 + self.loss2
        # self.loss = self.lossFn(out[0], colorlan) + self.clsloss(out[1], eyelabel1)
    ...
    def val_batch_with_ang_multi3(self):
        ...
        for idx_, data_ in enumerate(self.testdataLoader):
            ...
            eyelabel1 = data_[1].to(device)
            label1 = data_[2].to(device)
            ...
            with torch.no_grad():
                ...
                loss = self.lossFn(out[0], eyelabel1) + self.clsloss(out[1], label1.long())
                # loss = self.lossFn(out[0], eyelabel1, out[1], label2, out[2], label3, 
                #                         out[3], label4)
    ...
    def __call__(self):
        ...
        for i in range(epoch):
            ...
            savePath = "./models/" + "multi_epoch_" + str(i) + "_lmark_cl.pkl"
